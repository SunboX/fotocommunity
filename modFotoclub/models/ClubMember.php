...
class ClubMember extends DataObjectDecorator
{
	...
	function augmentDatabase() {
		...
		if( $exist > 0 ) {
			DB::query( "UPDATE `Member`, `ClubMember` " .
				...
				"`Member`.`Occupation` = `ClubMember`.`Occupation`," .
	...
	function getEditProfileFields($addmode = false)
	{
		...
		$personalDetailsFields = new CompositeField(
			new HeaderField("PersonalDetails", 'Profil bearbeiten'),
			//new LabelField('', 'Hier kannst Du Dein öffentliches Profil einrichten'),
			...
			new SimpleImageField("Avatar", 'Upload avatar ' . $gravatarText)
		);
	...
	function GetAvatar() {
		...
		$default = "forum/images/ClubMember_holder.gif";
		if(file_exists('themes/'. SSViewer::current_theme().'_forum/images/ClubMember_holder.gif')) {
			$default = 'themes/'. SSViewer::current_theme().'_forum/images/ClubMember_holder.gif';
		}
